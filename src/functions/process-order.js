...
export default async function (req) {
    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            if (!finalUserImageUrl && body.userImageBase64) {
                try {
                    const b64Data = body.userImageBase64.replace(/^data:image\/\w+;base64,/, '');
                    ...
                    const path = `user-uploads/${tempId}/reference.jpg`;
                    ...
                    if (!uploadErr && uploadData?.url) {
                        finalUserImageUrl = uploadData.url;
                    }
                } catch (e) {
                    console.warn('Backend base64 upload fallback error:', e.message);
                }
            }
            ...
            const orderInsert = {
                ...
                reference_image_url: finalUserImageUrl || template.reference_image_url || null,
        ...
        if (req.method === 'PUT') {
            const body = await req.json();
            const { orderId, action, confirmDecision, base64Image } = body;
            ...
            if (action === 'recover-order' && base64Image) {
                ...
                await client.database
                    .from('orders')
                    .update({
                        reference_image_url: uploadData.url,
                        image_task_id: null,
                ...
                const { data: genData, error: genErr } = await client.functions.invoke('generate-image', {
                ...
                return new Response(JSON.stringify({
                    success: true,
                    reference_image_url: uploadData.url,
                    generation: { data: genData, error: genErr }
                }), {
                    status: 200,
                    headers: { ...corsHeaders, 'Content-Type': 'application/json' }
                });
